...
To create a template, make a new directory in =skel-user-directory=. It can
...
Once you have created a project skeleton, use the =define-project-skeleton=
...
#+begin_src elisp
(define-project-skeleton "my-elisp-package")
#+end_src

This will add =my-elisp-package= to the list of available projects. You can now
create an instance by calling =M-x create-project my-elisp-package=. Skeletor
...
The title is the string that represents the project in the =create-project=
prompt.
...
configuration. Skeletor provides the =skel-shell-command= and
=skel-async-shell-command= functions for this purpose. These functions output to
...
The =skel-global-substitutions= variable defines the substitutions available to
...
#+begin_src elisp
(add-to-list 'skel-global-substitutions
...
#+begin_src elisp
(define-project-skeleton "my-package"
  :substitutions
...
Skeletor provides the =define-skeleton-constructor= macro for this purpose. It
is similar to =define-project-skeleton=, but it requires you supply a function
